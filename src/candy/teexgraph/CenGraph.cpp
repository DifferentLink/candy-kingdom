...
// compute all closeness centrality values
vector<double> Graph::closenessCentrality(const Scope scope = LWCC, const double inputsamplesize = 1.0) {
    ...
    double total = 0;
    vector<long> longarray(vector<long>(nodes(FULL), 0));
    ...
    for(int i = 0; i < nodes(FULL); i++) {
        alllongarray[i] += (double) longarray[i];
        total += (i * longarray[i]);
    }
